...
vueApp = new Vue({
    ...
    watch: {
        gridSize: function (val) {
            if (typeof(val) === 'string') {
                val = this.gridSize = parseInt(val); // recursion !!!
                return;
            }
    ...
    methods: {
        ...
        setClickedCell: function (cellIdx, event) {
            if (this.gameStarted) {
                ...
                if (this.mouseTracking) {
                    ...
                    var nx = (event.pageX - rect.x) / this.$el.clientWidth;  // normalize in [0, 1] interval
        ...
        appendMouseMove: function(event) {
            if (this.mouseTracking) {
                var rect = this.$el.getBoundingClientRect();
                var nx = (event.clientX - rect.x) / this.$el.clientWidth;  // normalize in [0, 1] interval
