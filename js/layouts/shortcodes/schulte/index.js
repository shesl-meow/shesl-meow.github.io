(() => {
  ...
  var appData = {
    ...
    currGroup: 0,
    groups: [],
    // array of Group: setups in makeGridCells() method
    ...
    stats: {
      ...
      clear: function() {
        ...
        this.stopTime = /* @__PURE__ */ new Date();
      ...
      addClick: function(groupN, number, err, inverse, divergent) {
        ...
        var time = ((currTime - this.lastTime) / 1e3).toFixed(2);
    ...
    watch: {
      gridSize: function(val) {
        if (typeof val === "string") {
          val = this.gridSize = parseInt(val);
          return;
        }
        this.colWidth = 100 / val + "%";
        this.rowHeight = this.$el.offsetWidth / val + "px";
        this.initGame();
      },
      groupCount: function(val) {
        if (typeof val === "string") {
          this.groupCount = parseInt(val);
          return;
        }
        this.initGame();
      },
    ...
    methods: {
      ...
      startGame: function() {
        ...
        if (this.timerMode) {
          ...
          this.gameTimerId = setTimeout(this.gameTimerOut, this.timerMinutes * 60 * 1e3);
        }
      ...
      stopGame: function() {
        ...
        this.stats.stopTime = /* @__PURE__ */ new Date();
        this.gameStarted = false;
      ...
      setClickedCell: function(cellIdx, event) {
        if (this.gameStarted) {
          ...
          if (this.mouseTracking) {
            ...
            var nx = (event.pageX - rect.x) / this.$el.clientWidth;
            var ny = (event.pageY - rect.y) / this.$el.clientHeight;
      ...
      isCellCorrect: function(cellIdx) {
        return this.cells[cellIdx].group === this.currGroup && this.cells[cellIdx].number === this.groups[this.currGroup].currNum;
      },
      ...
      nextGroup: function() {
        this.currGroup = (this.currGroup + 1) % this.groupCount;
      },
      ...
      makeGridCells: function() {
        ...
        if (this.variousCounts) {
          var various = [
            { divergent: false, inverted: false, num: "first" },
      ...
      appendMouseMove: function(event) {
        if (this.mouseTracking) {
          var rect = this.$el.getBoundingClientRect();
          var nx = (event.clientX - rect.x) / this.$el.clientWidth;
          var ny = (event.clientY - rect.y) / this.$el.clientHeight;
          this.mouseMoves.push(new Point(nx, ny));
        }
      },
      drawMousemap: function() {
        var canvas = this.$refs["mousemap_canvas"];
        if (canvas) {
